class gravity {
  ...
  buildStars() {
    for (let i = 0; i < this.sn; i ++) {
      if (this.isFixed === true) {
        this.stars.push(new star(true, 0, 0));
      } else {
        this.stars.push(new star(false, this.C.x, this.C.y));
  ...
  buildAsteroids() {
    for (let i = 0; i < this.an; i ++) {
      if (this.isFixed === true) {
        this.asteroids.push(new asteroid(7, 1, 0.2, true));
      } else {
        this.asteroids.push(new asteroid(7, 1, 0.2, false));
      }
    }
  }
  ...
  getAsteroidAcceleration(attractor, object) {
	...
	setAsteroidsSpeed(x, y) {
		...
		let m = p5.Vector.sub(v, this.asteroids[0].p);
		for (let i = 0; i < this.an; i ++) {
      this.asteroids[i].setSpeed(v, m);
    }
	}
